...
function serialize(m, client) {
	...
	if (m.message) {
		...
		m.error = async (txt) => {
			let error = await client.sendMessage(m.jid, {
				text: `${txt}`
			}, {
				quoted: m
			})
		...
		m.forward = async (targetJid, message, options = {}) => {
			let contentType;
			let content = message;
			...
			const waMessage = generateWAMessageFromContent(targetJid, forwardContent, options ? {
		...
		m.sendMsg = async (jid, content, opt = {
		...
		}, type = "text") => {
			{
				switch (type.toLowerCase()) {
					...
					case "interactive": {
						const genMessage = await createInteractiveMessage(jid, content);
						await client.relayMessage(jid, genMessage.message, {
		...
		m.sendPoll = (jid, name = '', values = [], selectableCount = 1) => {
			return client.sendMessage(jid, {
				poll: {
					name,
					values
